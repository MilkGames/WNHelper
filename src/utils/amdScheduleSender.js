...
async function cleanupPreviousSchedule({ guildId, channel }) {
    ...
    try {
        const message = await runDiscordRequest(() => channel.messages.fetch(existing.messageId));
        await editMessageWithRetry(message, { components: [] }).catch(() => {});
    } catch (error) {
    ...
    for (let i = 0; i < stageDelays.length; i += 1) {
        ...
        setTimeout(async () => {
            try {
                ...
                const message = await runDiscordRequest(() => channel.messages.fetch(messageId));
                await editMessageWithRetry(message, { content }).catch(() => {});
                ...
                if (newlyAdded.length) {
                    await sendMessageWithRetry(channel, {
                        content: `:bell: Смены на ${formattedDate}: открыт доступ к выбору смен для ${formatRoleMentions(newlyAdded)}.`,
                    }, {
                        nonceSeed: `amdStage:${guildId}:${messageId}:${stageIdx}`,
                    });
                }
            } catch (error) {
...
async function sendAmdScheduleNow({ client, guildId, serverConfig, reason = 'manual' }) {
    let channel;
    try {
        channel = await client.channels.fetch(serverConfig.amdShiftsChannelId);
    } catch (error) {
        throw new Error(`Не удалось получить канал смен AMD: ${error}`);
    }

    const serverName = client.guilds.cache.get(guildId)?.name || guildId;
    logger.info(`Начинаю отправлять смены AMD для сервера ${serverName}. Причина: ${reason}`);
    ...
    const sentMessage = await sendMessageWithRetry(channel, {
        ...
        nonceSeed: `amdSchedule:${guildId}:${formattedDate}`,
    });
    ...
    logger.info(`Смены для AMD успешно отправлены на сервере ${serverName}. Причина: ${reason}`);

    return {
        channelId: sentMessage.channelId,
        messageId: sentMessage.id,
        formattedDate,
        messageLink: `https://discord.com/channels/${guildId}/${sentMessage.channelId}/${sentMessage.id}`,
    };
}
